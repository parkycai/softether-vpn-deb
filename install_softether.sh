...
# Downloads .deb files from the latest GitHub release (e.g., v5.02.5187-deb) using browser_download_url
# Downloads .service files via jsDelivr CDN (@latest) with GitHub raw as backup
# Dynamically detects .deb package names and version from release tag using bash regex
...
# Displays version number from release tag in the menu (e.g., 5.02.5187)
...
# Base URLs for downloads (.service files: jsDelivr primary, GitHub raw backup)
...
# Associative arrays to store detected .deb package names, version, and URLs
...
fetch_deb_packages() {
    ...
    local version
    version=$(echo "$release_tag" | sed -n 's/v\([0-9]\+\.[0-9]\{2\}\.[0-9]\+\)-deb/\1/p')
    if [ -z "$version" ]; then
    ...
    # Parse .deb files for each component from release assets
    for component in "common" "vpncmd" "${COMPONENTS[@]}"; do
        # Extract asset name containing softether-${component}_*_amd64.deb
        local deb_file
        deb_file=$(grep -o '"name":\s*"softether-'${component}'_[^"]*_amd64\.deb"' "$temp_json" | sed 's/"name":\s*"\(.*\)"/\1/' | head -n 1)
        if [ -z "$deb_file" ]; then
        ...
        # Extract GitHub download URL
        local github_url
        github_url=$(grep '"browser_download_url":.*softether-'${component}'_[^"]*_amd64\.deb' "$temp_json" | sed 's/.*"browser_download_url":\s*"\([^"]*\)".*/\1/' | head -n 1)
        if [ -z "$github_url" ]; then
            echo "No GitHub download URL found for ${deb_file}"
            rm -f "$temp_json"
            exit 1
        fi
        DEB_URLS["$component"]="$github_url"
    done
...
display_menu() {
    ...
    echo "3) vpnclient + vpnserver (v${DEB_VERSIONS[vpnserver]})"
...
download_components() {
    # Download mandatory .deb components (common and vpncmd) from GitHub
    echo "Downloading mandatory components (common and vpncmd) from GitHub..."
    for component in "common" "vpncmd"; do
        download_file "${DEB_URLS[$component]}" "${DEB_PACKAGES[$component]}"
    done

    # Download selected components' .deb files from GitHub
